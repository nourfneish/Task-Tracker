...
**Story 4 — Combine filters**
As a user, I want to combine search, status, priority, and assignee filters at the same time so that I can narrow results precisely (e.g. "High priority, In Progress, assigned to Alice").
- Acceptance criteria:
  - Filters are ANDed together, e.g. `GET /tasks?status=ToDo&priority=High` returns only tasks matching both.
  - Clearing filters restores the full board.

**Story 5 — Clear filters**
As a user, I want a single "clear filters" control so that I can reset the board quickly instead of clearing each field by hand.
- Acceptance criteria:
  - The clear button resets search, status, priority, and assignee inputs and re-fetches the unfiltered task list.
...
**Story 4 — Log deletion**
As a user, I want deleted tasks to leave a record so that "the task disappeared" isn't a mystery later.
- Acceptance criteria:
  - `DELETE /tasks/{id}` logs a `deleted` event with the task's id and title *before* removing it from storage.
  - The event remains visible in `GET /activity` after the task itself is gone (activity is not deleted along with the task).

**Story 5 — View activity per task and globally**
As a user, I want to see a global activity feed and also a feed scoped to one task so that I can either monitor the whole board or audit a single task's history.
- Acceptance criteria:
  - `GET /activity` returns all events, newest first.
  - `GET /tasks/{id}/activity` returns only that task's events, newest first, and returns `404` if the task id doesn't exist.
